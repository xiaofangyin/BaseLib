...
public final class ViewfinderView extends View {
    ...
    private boolean isBarCode;//是否是条形码
    ...
    public ViewfinderView(Context context, AttributeSet attrs) {
        ...
        labelTextSize = array.getFloat(R.styleable.ViewfinderView_label_text_size, DensityUtil.sp2px(getContext(), 14));
    ...
    public void onDraw(Canvas canvas) {
        Rect frame = CameraManager.get().getFramingRect(isBarCode);
        if (frame == null) {
            return;
        }
    ...
    private void drawTextInfo(Canvas canvas, Rect frame) {
        ...
        canvas.drawText(labelText, frame.left + frame.width() / 2f, frame.bottom + CORNER_RECT_HEIGHT * 2.5f, paint);
    }
